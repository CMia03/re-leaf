...
import { SIGN_UP, UPDATE_USER } from "@/graphql/queries/authServices";
...
const SignUp = () => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const jwt = registerData?.register?.jwt;
      const userId = registerData?.register?.user?.id;

      if (!jwt || !userId) {
        toast.error("Inscription échouée : Informations manquantes.");
        return;
      }
  ...
  return (
    <div className="w-full p-6 px-0 ps-15">
      <div className="flex flex-col gap-8">
        ...
        >
          ...
          >
            <Typography variant="D1">{t("createAccount")}</Typography>
          </Button>
        </form>
      </div>
    </div>
